 ...
 * Example usage:
 * <pre>
 * // Create a generator for random integers between 1 and 10
 * Gen<Integer> randomIntGen = r -> () -> r.nextInt(10) + 1;
 * Integer randomInt = randomIntGen.sample(new Random()).get();
 * </pre>
 * Note: Implementations of this interface should ensure thread-safety when used in a multi-threaded environment.
 *
...
public interface Gen<O> extends Function<Random, Supplier<O>> {
    ...
    static <O> String tag(final O val,
                          final List<Pair<Predicate<O>, String>> classifiers) {
        return classifiers.stream()
                          .filter(cla -> cla.first().test(val))
                          .map(Pair::second)
